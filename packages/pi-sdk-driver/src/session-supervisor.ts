...
import type { SessionCatalogSnapshot, WorkspaceCatalogSnapshot } from "@bimanus/catalogs";
...
export class SessionSupervisor {
  ...
  async createSession(workspace: WorkspaceRef, options?: CreateSessionOptions): Promise<SessionSnapshot> {
    ...
    const appendSystemPrompt = (options as Record<string, unknown> | undefined)?.appendSystemPrompt;
    if (typeof appendSystemPrompt === "string" && appendSystemPrompt.length > 0) {
      (createOptions as PiCreateAgentSessionOptions & { resourceLoaderOptions?: Record<string, unknown> }).resourceLoaderOptions = {
  ...
  async getSessionSnapshot(sessionRef: SessionRef): Promise<SessionSnapshot> {
    const record = await this.ensureRecord(sessionRef);
    await this.touchWorkspace(record.workspace);
  ...
  async cancelCurrentRun(sessionRef: SessionRef): Promise<void> {
    ...
    const record = this.records.get(key) ?? await this.ensureRecord(sessionRef).catch(() => undefined);
    if (!record?.session) {
      return;
    }
  ...
  async reopenSessionFromDisk(sessionRef: SessionRef): Promise<void> {
    const record = await this.ensureRecord(sessionRef);
    const sessionFile = record.sessionFile ?? record.session?.sessionFile ?? record.session?.sessionManager.getSessionFile();
    if (!sessionFile) {
      throw new Error(`Session ${sessionKey(sessionRef)} cannot be reopened because no session file is tracked.`);
    }
    ...
    const runtime = await this.createAgentSessionRuntimeImpl({
      ...
      sessionManager: openPersistedSessionManager(record.workspace.path, sessionFile),
      ...(this.modelRegistry ? { modelRegistry: this.modelRegistry } : {}),
    });
  ...
  private async ensureRecord(sessionRef: SessionRef): Promise<ManagedSessionRecord> {
    ...
    const runtime = await this.createAgentSessionRuntimeImpl({
      cwd: workspace.path,
      sessionManager: openPersistedSessionManager(workspace.path, sessionFile),
      ...(this.modelRegistry ? { modelRegistry: this.modelRegistry } : {}),
    });
    const session = runtime.session;
  ...
  private async syncRecordAfterSessionMutation(
    ...
    record.status = session.isStreaming && !record.cancelRequested ? "running" : "idle";
    record.runningRunId = session.isStreaming && !record.cancelRequested ? record.runningRunId ?? crypto.randomUUID() : undefined;
    record.config = deriveSessionConfig(session.sessionManager);
    record.preview =
  ...
  private mapAgentEvent(record: ManagedSessionRecord, event: AgentSessionEvent): SessionDriverEvent[] {
    ...
    switch (event.type) {
      ...
      case "agent_end": {
        ...
        const cancelRequested = record.cancelRequested;
        const runId = record.runningRunId;
        record.runningRunId = undefined;
        record.cancelRequested = false;

        // An intentional user-initiated abort (stopReason === "aborted") is a
        // clean cancellation, not a failure.  Treating it as "failed" would
        // leave the UI in an error state after the user clicked Stop.
        // If cancelCurrentRun already set the record to "idle" before this
        // event arrives (the optimistic path), preserve that status so the
        // race between the optimistic update and this event is always safe.
        const isAborted = cancelRequested || (!outcome.success && outcome.error?.code === "ABORTED");
        if (record.status !== "idle") {
          record.status = outcome.success || isAborted ? "idle" : "failed";
        }
        record.updatedAt = timestamp;
        if (!outcome.success && outcome.error && !isAborted) {
...
function openPersistedSessionManager(cwd: string, sessionFile: string): SessionManager {
