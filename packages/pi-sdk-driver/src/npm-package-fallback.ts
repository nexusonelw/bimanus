...
import { configureSettingsManagerForDesktopRuntime, createDesktopRuntimeSettingsManager } from "./runtime-deps.js";
...
export function isGlobalNpmLookupError(error: unknown): boolean {
  ...
  return (
    message.includes("npm root -g") ||
    message.includes("spawn npm ENOENT") ||
    message.includes("spawnSync npm ENOENT") ||
    /Failed to run npm(?:\s|$).*ENOENT/i.test(message)
  );
}
...
export function createSettingsManagerWithoutNpmPackages(current: SettingsManager): SettingsManager | null {
  ...
  return configureSettingsManagerForDesktopRuntime(
...
async function createAgentSessionServicesWithNpmFallback(
  ...
  try {
    ...
    const currentSettingsManager = options?.settingsManager ?? createDesktopRuntimeSettingsManager(cwd, agentDir);
...
async function createAgentSessionResultWithNpmFallback(
  ...
  const settingsManager = options?.settingsManager ?? createDesktopRuntimeSettingsManager(cwd, agentDir);
  const services = await createAgentSessionServicesWithNpmFallback(cwd, agentDir, {
    ...options,
    settingsManager,
  });
  return {
