...
async function requestNotificationPermissionInternal(
  ...
  if (process.platform === "darwin" && app.isPackaged) {
    if (override) {
      ...
      return await readNotificationPermissionStatus(window);
    }
  ...
  if (!canAccessRendererNotificationPermission(window)) {
    if (override) {
...
function canAccessRendererNotificationPermission(window: BrowserWindow | null): window is BrowserWindow {
  if (!window || window.isDestroyed()) {
    return false;
  }
  const { webContents } = window;
  return !webContents.isDestroyed() && !webContents.isCrashed() && !webContents.isLoading();
}
