...
export async function resolvePackagedAppBundle(releaseDir = packagedReleaseDir): Promise<string> {
  ...
  try {
    ...
    if (isMissingPathError(error)) {
      throw new Error(
        `Packaged release directory not found: ${releaseDir}. Run pnpm --filter @bimanus/desktop run package:dir first.`,
      );
    }
    throw error;
  }

  const appBundle = appBundles.find((candidate) => basename(candidate) === "Bimanus.app") ?? appBundles[0];
  if (!appBundle) {
    throw new Error(`No .app bundle found under ${releaseDir}. Run pnpm --filter @bimanus/desktop run package:dir first.`);
  }
...
export async function resolvePackagedReleaseZip(releaseDir = packagedReleaseDir): Promise<string> {
  ...
  if (!zipEntry) {
    throw new Error(`No packaged macOS release zip found under ${releaseDir}. Run pnpm --filter @bimanus/desktop run package first.`);
  }
