...
export function ModelSelector({
  ...
  const { t } = useI18n();
  const resolvedUnselectedLabel = unselectedModelLabel ?? t("modelSelector.choose");
  const resolvedEmptyLabel = emptyModelLabel ?? t("modelSelector.choose");
  ...
  const modelBadgeLabel = provider && modelId ? `${provider}:${modelId}` : hasAvailableModelOptions ? resolvedUnselectedLabel : resolvedEmptyLabel;
