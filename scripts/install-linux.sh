...
write_env_file() {
  local env_file="$1"
  # Servers without a display should run remote-only (existing RemoteUiServer),
  # not open a local Electron window or require Xvfb.
  {
    printf '%s\n' "# Generated by Bimanus Linux installer. Sourced by the bimanus launcher."
    printf '%s\n' "PI_APP_REMOTE_UI=1"
    printf 'PI_APP_REMOTE_UI_HOST=%s\n' "$HOST"
    printf 'PI_APP_REMOTE_UI_PORT=%s\n' "$PORT"
    # %q keeps special characters in the token safe under set -u / source.
    printf 'PI_APP_REMOTE_UI_TOKEN=%q\n' "$TOKEN"
...
write_launcher() {
  ...
  local env_file="$3"
  # Build with printf + quoted heredoc so set -u never expands outer $@ / $1 / $2.
  {
    printf '%s\n' "#!/usr/bin/env bash"
    printf '%s\n' "set -euo pipefail"
    printf 'APPIMAGE=%q\n' "$appimage_path"
...
# Default to headless remote-only mode on machines without a display.
# Reuses the app's existing Remote UI bridge (no second implementation).
if [[ -z "${DISPLAY:-}" && -z "${WAYLAND_DISPLAY:-}" && -z "${PI_APP_HEADLESS:-}" ]]; then
  export PI_APP_HEADLESS=1
fi
extra_args=()
if [[ "${PI_APP_HEADLESS:-}" == "1" ]]; then
  extra_args+=(--headless --no-sandbox)
fi
...
EOF
  ...
  printf '\n'
  info "Start on this machine (remote-only, no local GUI window):"
  ...
  printf '\n'
  info "$(bold "Runtime libraries (not a desktop GUI)")"
  info "Headless mode does NOT need Xvfb/DISPLAY, but the AppImage still needs shared libs:"
  info "  # Debian / Ubuntu"
